...
export default function NewZealandPage() {
  ...
  useEffect(() => {
    getArticlesByDomain('hockeynz.co.nz', 20)
  ...
  if (!articles.length) return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Hockey New Zealand articles will appear here once the scraper runs.</p>
    </div>
  )
  ...
  return (
    <>
      <style>{`
        ...
        .nz-wrap     { max-width:1200px; margin:0 auto; padding:24px 24px 100px; animation:fadeUp .4s ease; }
        ...
        .nz-card-hover { transition: transform .25s ease, box-shadow .25s ease; }
        .nz-card-hover:hover { transform: translateY(-4px); box-shadow: 0 20px 60px rgba(0,0,0,.5); }
        .nz-img-zoom img { transition: transform .6s ease; }
        .nz-img-zoom:hover img { transform: scale(1.05); }
        .nz-title-hover { transition: color .2s ease; }
        .nz-card-hover:hover .nz-title-hover { color: var(--accent) !important; }
      `}</style>
      ...
      <div className="nz-wrap">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
          ...
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 900, margin: 0, letterSpacing: -0.5 }}>Hockey New Zealand</h1>
          </div>
        </div>

        {hero && <NZHeroCard article={hero} />}

        {secondary.length > 0 && (
          <div className="sec-grid" style={{ marginTop: 16, marginBottom: 48 }}>
            {secondary.map(a => <NZSecondaryCard key={a.id} article={a} />)}
          </div>
        )}
        ...
        {grid.length > 0 && (
          <>
            ...
            <div className="news-grid" style={{ marginBottom: 60 }}>
              {grid.map(a => <NZGridCard key={a.id} article={a} />)}
            </div>
          </>
        )}
      </div>
...
function NZHeroCard({ article }: { article: Article }) {
    ...
    >
      <div style={{ position: 'relative', height: 520, overflow: 'hidden', borderRadius: 4, cursor: 'pointer' }}>
        ...
        <div style={{ position: 'absolute', top: 24, left: 24, display: 'flex', gap: 8 }}>
          ...
          <span style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)', color: '#fff', fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', padding: '5px 10px', borderRadius: 6 }}>🇳🇿 New Zealand</span>
        </div>
...
function NZSecondaryCard({ article }: { article: Article }) {
  const slug  = getSlug(article)
  const title = getTitle(article)

  return (
    <Link href={`/article/${slug}`} style={{ textDecoration: 'none' }}>
      <div className="nz-card-hover nz-img-zoom" style={{ borderRadius: 6, overflow: 'hidden', background: 'var(--bg-card)', border: '1px solid var(--border)', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '16px 18px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 1.2, textTransform: 'uppercase', color: 'var(--tag-text)', background: 'var(--tag-bg)', padding: '3px 7px', borderRadius: 4, border: '1px solid var(--border)' }}>🇳🇿 New Zealand</span>
          ...
          <h3 className="nz-title-hover" style={{ fontSize: 15, fontWeight: 800, lineHeight: 1.35, color: 'var(--text-primary)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {title}
          </h3>
        </div>
      </div>
...
function NZGridCard({ article }: { article: Article }) {
  const slug  = getSlug(article)
  const title = getTitle(article)

  return (
    <Link href={`/article/${slug}`} style={{ textDecoration: 'none' }}>
      <div className="nz-card-hover nz-img-zoom" style={{ borderRadius: 6, overflow: 'hidden', background: 'var(--bg-card-2)', border: '1px solid var(--border)', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '13px 15px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--tag-text)', background: 'var(--tag-bg)', padding: '2px 6px', borderRadius: 4, border: '1px solid var(--border)' }}>🇳🇿 New Zealand</span>
            <div style={{ width: 3, height: 3, borderRadius: '50%', background: 'var(--green-text)' }} />
          ...
          <h3 className="nz-title-hover" style={{ fontSize: 14, fontWeight: 800, lineHeight: 1.35, color: 'var(--text-primary)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {title}
          </h3>
        </div>
      </div>
